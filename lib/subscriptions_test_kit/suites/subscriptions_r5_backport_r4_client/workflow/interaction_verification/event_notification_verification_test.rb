    ...
    class EventNotificationVerificationTest < Inferno::Test
      ...
      run do
        ...
        if subscription.present?
          ...
          assert(request.content_type_header == mime_type, "Content type of request does not match the Subscription MIME type")
          # Verification for hl7.fhir.uv.subscriptions_1.1.0@27
          assert(request.header.include?(subscription&.channel&.header), "Subscriptoin channel header is not conveyed as HTTP request header")
