require_relative '../../../../lib/subscriptions_test_kit/suites/subscriptions_r5_backport_r4_server/' \
                 'subscription_rejection/reject_subscription_filter_test'

RSpec.describe SubscriptionsTestKit::SubscriptionsR5BackportR4Server::RejectSubscriptionFilterTest do
  ...
  describe 'Server Subscription Rejects Unsupported Filter Test' do
    let(:test) do
      Class.new(SubscriptionsTestKit::SubscriptionsR5BackportR4Server::RejectSubscriptionFilterTest) do
    ...
    it 'fails if a Subscription requests returns a 201 response and did not alter the Subscription resource' do
      ...
      subscription_resource['_criteria'] = { 'extension' => [{
      ...
      expect(entity_result_message(test)).to match('Subscription with unsupported filter criteria')
    end
  end
end
