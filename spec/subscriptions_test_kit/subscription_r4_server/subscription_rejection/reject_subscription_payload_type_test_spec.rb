require_relative '../../../../lib/subscriptions_test_kit/suites/subscriptions_r5_backport_r4_server/' \
                 'subscription_rejection/reject_subscription_payload_type_test'
...
RSpec.describe SubscriptionsTestKit::SubscriptionsR5BackportR4Server::RejectSubscriptionPayloadTypeTest do
  ...
  describe 'Server Subscription Rejects Unsupported Payload Type Test' do
    let(:test) do
      Class.new(SubscriptionsTestKit::SubscriptionsR5BackportR4Server::RejectSubscriptionPayloadTypeTest) do
    ...
    it 'passes if Subscription request returns a non 201 response' do
      ...
      result = run(test, server_endpoint:, server_credentials:, subscription_resource: subscription_resource.to_json,
                         unsupported_subscription_payload_type:)
    ...
    it 'fails if a Subscription requests returns a 201 response and did not alter the Subscription resource' do
      ...
      subscription_resource['channel']['payload'] = unsupported_subscription_payload_type
      ...
      expect(entity_result_message(test)).to match('Subscription with unsupported payload type')
    end
  end
end
